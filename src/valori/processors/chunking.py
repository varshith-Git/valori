...
class ChunkingProcessor(DocumentProcessor):
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        self.strategy = config.get(
            "strategy", "fixed_size"
        )  # fixed_size, semantic, sliding_window
    ...
    def _semantic_chunking(self, text: str) -> List[Dict[str, Any]]:
        ...
        for paragraph in paragraphs:
            ...
            if current_chunk and len(current_chunk) + len(paragraph) > self.chunk_size:
                ...
                current_start = max(
                    0, current_start + len(current_chunk) - self.chunk_overlap
                )
            else:
            ...
            chunks.append(
                {
                    ...
                    "end_pos": current_start + len(current_chunk),
                    "length": len(current_chunk.strip()),
                    "metadata": {"chunk_type": "semantic", "strategy": self.strategy},
                }
            )
    ...
    def get_stats(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "average_chunk_size": self._total_characters_processed
            / max(self._chunks_created, 1),
