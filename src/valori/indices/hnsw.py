...
class HNSWIndex(Index):
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        self.levels: List[Dict[str, List]] = (
            []
        )  # Each level is a dict of node -> neighbors
    ...
    def insert(
        ...
        try:
            ...
            for i, vector in enumerate(vectors):
                ...
                vector_metadata = (
                    metadata[i] if metadata and i < len(metadata) else None
                )
                ...
                sb.store_vector(vector_id, vector, vector_metadata)
        ...
        try:
            ...
            if current_level >= 0 and self.levels[current_level]:
                ...
                candidates = [
                    (
                        entry_point,
                        self._compute_distance(query_vector, self.vectors[entry_point]),
                    )
                ]
                        ...
                        for neighbor_id in self.levels[current_level][node_id]:
                            if neighbor_id in self.vectors:
                                distance = self._compute_distance(
                                    query_vector, self.vectors[neighbor_id]
                                )
            ...
            for node_id, distance in top_k:
                _, metadata = sb.retrieve_vector(node_id)
                results.append(
                    {
                        "id": node_id,
                        "distance": distance,
    ...
    def delete(self, ids: List[str]) -> bool:
        ...
        try:
            ...
            for vector_id in ids:
                # Remove from storage
                if not sb.delete_vector(vector_id):
                    success = False
    ...
    def update(
        ...
        try:
            ...
            if not sb.update_vector(id, vector, metadata):
                return False
