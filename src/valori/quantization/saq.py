...
class SAQQuantizer(Quantizer):
            ...
            for end in range(seg, dim + 1):
                for start in range(seg - 1, end):
                    if start == 0:
                        ...
                        seg_variance = cumulative_variance[end - 1] - cumulative_variance[start - 1]
    ...
    def quantize(self, vectors: np.ndarray) -> np.ndarray:
        ...
        try:
            ...
            quantized_segments = []
            for (start, end), quantizer in zip(self.segment_boundaries, self.segment_quantizers):
                segment_vectors = vectors_pca[:, start:end]
    ...
    def _apply_code_adjustment(self, vectors_pca: np.ndarray, 
                             quantized_segments: List[np.ndarray],
                             original_vectors: np.ndarray) -> np.ndarray:
        """
        ...
        for iteration in range(self.adjustment_iters):
            ...
            for seg_idx in range(len(self.segment_quantizers)):
                ...
                reconstructed = self._dequantize_segments(best_codes, exclude_segment=seg_idx)
                ...
                new_codes = quantizer.quantize(segment_residual + reconstructed[:, start:end])
    ...
    def _dequantize_segments(self, codes: np.ndarray, 
                           exclude_segment: Optional[int] = None,
                           updated_segment: Optional[Tuple[int, np.ndarray]] = None) -> np.ndarray:
    ...
    def search_with_rescoring(self, query: np.ndarray, quantized_vectors: np.ndarray,
        ...
        if self.full_precision_vectors is None:
            raise QuantizationError("Full precision vectors not available for rescoring")
    ...
    def _initial_quantized_search(self, query: np.ndarray, 
    ...
    def _rescoring_search(self, query: np.ndarray, candidate_indices: np.ndarray,
