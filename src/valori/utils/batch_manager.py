...
class BatchManager:
    ...
    def __init__(self, batch_size: int = 1000, max_workers: int = 4):
        ...
        self._stats: Dict[str, Optional[Union[int, float]]] = {
            "total_items": 0,
            "processed_items": 0,
            "failed_items": 0,
            "start_time": None,
                ...
                try:
                    ...
                    if self.progress_callback:
                        self.progress_callback(
                            self._stats["processed_items"], self._stats["total_items"]
                        )
    ...
    def batch_search(
        self, client, query_vectors: np.ndarray, k: int = 10, show_progress: bool = True
    ...
    def parallel_batch_insert(
        ...
        def process_batch(batch_data):
            ...
            try:
                batch_ids = client.insert(batch_data["vectors"], batch_data["metadata"])
    ...
    def _print_progress(self):
        ...
        print(
            f"\rProgress: {percentage:.1f}% ({self._stats['processed_items']}/{self._stats['total_items']}) "
    ...
    def get_stats(self) -> Dict[str, Any]:
        ...
        if stats["start_time"] and stats["end_time"]:
            total_time: float = stats["end_time"] - stats["start_time"]
            stats["total_time"] = total_time
            processed_items: int = int(stats.get("processed_items", 0) or 0)
            if processed_items > 0 and total_time:
                stats["items_per_second"] = processed_items / total_time
            else:
...
class ResourceMonitor:
    ...
    def get_summary(self) -> Dict[str, Any]:
        ...
        return {
            "duration": self.resource_data[-1]["timestamp"]
    ...
    def save_report(self, filepath: Union[str, Path]):
        ...
        report = {"summary": self.get_summary(), "data": self.resource_data}
