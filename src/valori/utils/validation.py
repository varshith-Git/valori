...
def validate_vector(
    ...
    if min_dim is not None and len(vector) < min_dim:
        raise ValidationError(
            f"Vector dimension {len(vector)} is less than minimum {min_dim}"
        )

    if max_dim is not None and len(vector) > max_dim:
        raise ValidationError(
            f"Vector dimension {len(vector)} is greater than maximum {max_dim}"
        )

    if dtype is not None and vector.dtype != dtype:
        raise ValidationError(
            f"Vector dtype {vector.dtype} does not match expected {dtype}"
        )
...
def validate_vectors(
    ...
    if min_dim is not None and vectors.shape[1] < min_dim:
        raise ValidationError(
            f"Vector dimension {vectors.shape[1]} is less than minimum {min_dim}"
        )

    if max_dim is not None and vectors.shape[1] > max_dim:
        raise ValidationError(
            f"Vector dimension {vectors.shape[1]} is greater than maximum {max_dim}"
        )

    if dtype is not None and vectors.dtype != dtype:
        raise ValidationError(
            f"Vectors dtype {vectors.dtype} does not match expected {dtype}"
        )
        ...
        for key, expected_type in value_types.items():
            if key in config:
                if not isinstance(config[key], expected_type):
                    raise ValidationError(
                        f"Key '{key}' must be of type {expected_type}, got {type(config[key])}"
                    )
            ...
            if key in config:
                ...
                if isinstance(value, (int, float)):
                    if value < min_val or value > max_val:
                        raise ValidationError(
                            f"Key '{key}' value {value} is outside range [{min_val}, {max_val}]"
                        )
...
def validate_metadata(metadata: Optional[Dict[str, Any]]) -> bool:
    ...
    for key, value in metadata.items():
        ...
        try:
            ...
            raise ValidationError(
                f"Metadata value for key '{key}' is not JSON serializable"
            )
