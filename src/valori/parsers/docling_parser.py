...
class DoclingParser(DocumentParser):
    ...
    def parse(self, file_path: Union[str, Path]) -> Dict[str, Any]:
        ...
        try:
            ...
            for element in doc.iterate_items():
                ...
                if (
                    self.extract_tables
                    and hasattr(element, "label")
                    and "table" in element.label.lower()
                ):
                ...
                if (
                    ...
                    and hasattr(element, "label")
                    and "figure" in element.label.lower()
                ):
            ...
            raise ParsingError(
                f"Failed to parse document with Docling {file_path}: {str(e)}"
            )
    ...
    def _create_structured_chunks(
        self, text_elements: List[str], tables: List[Dict]
    ) -> List[Dict[str, Any]]:
