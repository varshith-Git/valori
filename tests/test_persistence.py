...
class TestTensorPersistence:
    ...
    def test_save_load_vectors(self, tensor_persistence, sample_vectors):
        ...
        # Save vectors
        success = tensor_persistence.save_vectors(
            sample_vectors, vector_ids, "test_vectors.npz"
        )
        assert success
    ...
    def test_compression(self, temp_dir, sample_vectors):
        ...
        success = persistence.save_vectors(
            sample_vectors, vector_ids, "compressed_vectors.npz"
        )
        assert success
...
class TestIncrementalPersistence:
    ...
    def test_save_load_state(self, incremental_persistence, sample_vectors):
        ...
        # Create test state
        state = {"vectors": sample_vectors, "metadata": {"count": len(sample_vectors)}}
    ...
    def test_save_load_vectors(self, incremental_persistence, sample_vectors):
        ...
        # Save vectors
        success = incremental_persistence.save_vectors(
            sample_vectors, vector_ids, "test_vectors.npz"
        )
        assert success

        # Load vectors
        loaded_vectors, loaded_ids = incremental_persistence.load_vectors(
            "test_vectors.npz"
        )
        assert loaded_vectors is not None
        assert loaded_ids is not None
    ...
    def test_operation_logging(self, incremental_persistence, sample_vectors):
        ...
        incremental_persistence.save_vectors(
            sample_vectors[:5], ["id1", "id2", "id3", "id4", "id5"], "vectors1.npz"
        )
        incremental_persistence.save_state({"test": "data"}, "state1.pkl")
        incremental_persistence.save_vectors(
            sample_vectors[5:], ["id6", "id7", "id8", "id9", "id10"], "vectors2.npz"
        )
    ...
    def test_get_stats(self, incremental_persistence, sample_vectors):
        ...
        incremental_persistence.save_vectors(
            sample_vectors, ["id1", "id2"], "vectors.npz"
        )
...
class TestPersistencePerformance:
    ...
    def test_large_data_handling(self, temp_dir, high_dim_vectors):
        ...
        # Save large vectors
        success = persistence.save_vectors(
            high_dim_vectors, vector_ids, "large_vectors.npz"
        )
        assert success
    ...
    def test_compression_vs_no_compression(self, temp_dir, sample_vectors):
        ...
        # No compression
        config_no_comp = {
            "data_dir": str(temp_dir / "no_compression"),
            "compression": False,
        }
