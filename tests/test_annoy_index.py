...
class TestAnnoyIndex:
    ...
    def setup_method(self):
        ...
        np.random.seed(42)
        self.vectors = np.random.randn(self.num_vectors, self.dimension).astype(
            np.float32
        )
        self.metadata = [
            {"id": i, "category": f"cat_{i%10}"} for i in range(self.num_vectors)
        ]
    ...
    def test_annoy_initialization(self):
        ...
        config = {"num_trees": 10, "metric": "angular", "search_k": -1}
    ...
    def test_annoy_different_metrics(self):
        ...
        for metric in metrics:
            config = {"num_trees": 5, "metric": metric}
    ...
    def test_annoy_on_disk_build(self):
        ...
        config = {"num_trees": 10, "metric": "angular", "build_on_disk": True}
        index = AnnoyIndex(config)
        index.initialize()
