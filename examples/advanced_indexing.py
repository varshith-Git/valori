...
def compare_indexing_algorithms():
    ...
    vectors, metadata, query_vectors = generate_test_data(
        num_vectors=5000, dimension=128, num_queries=50  # Reduced for faster demo
    )
    ...
    indices = [
        ...
        (
            IVFIndex({"metric": "cosine", "n_clusters": 100, "n_probes": 10}),
            "IVF Index",
        ),
        (
            LSHIndex(
        ...
        (
            AnnoyIndex({"metric": "angular", "num_trees": 10, "search_k": -1}),
            "Annoy Index",
        ),
    ]
    ...
    for index, name in indices:
        try:
            result = benchmark_index(
                index, vectors, metadata, query_vectors, k=10, name=name
            )
    ...
    print(f"{'='*80}")
    print(
        f"{'Index Name':<20} {'Add (vec/s)':<12} {'Search (ms)':<12} {'Throughput (q/s)':<16}"
    )
    print("-" * 80)
...
def demonstrate_lsh_features():
    ...
    # Generate test data
    vectors, metadata, query_vectors = generate_test_data(
        num_vectors=1000, dimension=64, num_queries=10
    )
    ...
    annoy_configs = [
        {
            ...
            "config": {"metric": "angular", "num_trees": 50, "search_k": 100},
        },
        {
            "name": "Fast Annoy",
            "config": {"metric": "angular", "num_trees": 5, "search_k": 10},
        },
        {
            "name": "On-Disk Annoy",
            "config": {"metric": "euclidean", "num_trees": 20, "build_on_disk": True},
        },
    ]
