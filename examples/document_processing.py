...
def setup_processing_pipeline():
    ...
    pipeline_config = {
        "parsers": {
            ...
            "pdf": {"extract_tables": True, "chunk_size": 1000, "chunk_overlap": 100},
            "office": {"extract_tables": True, "chunk_size": 800, "chunk_overlap": 80},
        },
        "processors": {
            "cleaning": {
                "remove_html": True,
        ...
        try:
            ...
            if "embedding" in result:
                ...
                metadata = {
                    ...
                    "text_length": len(result.get("text", "")),
                    "num_chunks": len(result.get("chunks", [])),
                    "parser_used": result.get("pipeline_metadata", {}).get(
                        "parser_used", "unknown"
                    ),
                }
