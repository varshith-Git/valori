...
def demo_helper_functions():
    ...
    print(
        f"Normalized vectors: mean norm = {np.mean(np.linalg.norm(normalized_vectors, axis=1)):.4f}"
    )
...
def demo_batch_operations():
    ...
    # Insert in batches
    inserted_ids = batch_manager.batch_insert(
        client, vectors, metadata, show_progress=True
    )
